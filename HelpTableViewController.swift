    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if tableView.cellForRowAtIndexPath(indexPath)!.textLabel?.text ==
            ...
            let alertVC = UIAlertController(title: "Always",
                message: "Your doctor can and will always be able to review your chat history to better monitor your progress. Although your providers will likely not read every conversation with Flo, they will periodically review your chat history and be queued into messages sent directly to them.", preferredStyle: .Alert)
            alertVC.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
            self.presentViewController(alertVC, animated: true, completion: nil)
        } else if tableView.cellForRowAtIndexPath(indexPath)!.textLabel?.text ==
            "Who is Flo?" {
            let alertVC = UIAlertController(title: "Hello!",
                message: "Flo is your personal medical assistant. We've designed her to respond to your questions, stay up-to-date on your treatment, answer your questions, communicate your progress, and be a mediator between you and your providers.", preferredStyle: .Alert)
            alertVC.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
            self.presentViewController(alertVC, animated: true, completion: nil)
        } else if tableView.cellForRowAtIndexPath(indexPath)!.textLabel?.text ==
            ...
            "Can I schedule appointments within Medchat?" {
            let alertVC = UIAlertController(title: "Yes, but only when necessary.",
                message: "You can request an appointment be scheduled from either your \"My Plan\" page, your Inbox (with a message to your provider), or by requesting Flo to schedule for you. We cannot confirm how quickly your providers will be able to accommodate.", preferredStyle: .Alert)
            ...
            let alertVC = UIAlertController(title: "Please keep your providers in the loop!",
                message: "You can stay in touch with your providers directly through your inbox and by sending messages directly to them through Flo.", preferredStyle: .Alert)
            alertVC.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
            self.presentViewController(alertVC, animated: true, completion: nil)
        } else if tableView.cellForRowAtIndexPath(indexPath)!.textLabel?.text ==
            ...
            let alertVC = UIAlertController(title: "What can we help you with?", message: "Please contact us at team@medchat.com from your inbox with any questions about how we could improve your experience on Medchat.", preferredStyle: .Alert)
